...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm03')
...
df['target'] = df['target'].apply(lambda x: 'HD' if x == 1 else 'noHM')
...
box_fig = px.box(df, x=box_cat, y=box_x, title="Box plot of " + box_x, color='target', 
                        color_discrete_map=dict(noHB = 'green', HB = 'red'), 
                        template="plotly_white", 
                        points="all") 
st.write(box_fig)
...
with row2_1: 
    corr_x = st.selectbox("Correlation - X variable", options=df.columns, index=df.columns.get_loc("trestbps"))
...
# correlation heatmap
st.subheader('상관성, 반상관성 높은 변수끼리 히트맵')
df3 = df.drop(columns=['sex', 'trestbps','chol','fbs','restecg'], axis=1)
